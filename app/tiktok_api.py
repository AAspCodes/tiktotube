...
class Tiktube:
    ...
    def is_hashtag_popular(self, hashtag: str) -> bool:
        ...
        return (
            hashtag_stats["videoCount"] >= self.min_video_count
            and hashtag_stats["viewCount"] >= self.min_view_count
        )

    def get_tiktoks_by_tags(
        self, hashtags: List, max_compilation_duration_seconds: int
    ) -> List:
        """
        Arguments:
        ...
        while total_duration_seconds < max_compilation_duration_seconds:
            ...
            for hashtag in hashtags:
                ...
                while True:
                    try:
                        video = self.api.by_hashtag(
                            hashtag[1:], count=1, offset=offset
                        )[0]
                        break
                    except Exception:
                        continue
        ...
        for video in videos:
            ...
            if video_hashtags:
                for hashtag in video_hashtags:
                    if self.is_hashtag_valid(hashtag) and self.is_hashtag_popular(
                        hashtag
                    ):
    ...
    def download_tiktoks(
        self,
        max_compilation_duration_seconds: int,
        hashtags_list: List = [],
        path_to_dloaded_vids: str = "downloaded_videos",
        video_prefix: str = "saved_video",
    ) -> None:
        ...
        self.downloader = Downloader(
            self.api, path_to_dloaded_vids, video_prefix="".join(hashtags_list)
        )
        # Get tiktoks basing on hashtags
        video_dicts = self.get_tiktoks_by_tags(
            hashtags_list, max_compilation_duration_seconds
        )
        if video_dicts:
